...
DEEPDIVER" experiment (§ V-C-3, Figure 16). Unlike Experiment 11 (Figure 12),
...
The produced plot mirrors Figure 16:
...
from Mups.MutualFuncs import level as pattern_level
...
class Experiment15Config:
    ...
    max_cardinality: int = 20               # auto-detect upper bound
    ...
    # --- plotting / output -----------------------------------------------------
    runtime_log_scale: bool = True          # Figure 16 uses a log y-axis
    show: bool = False
    save: bool = True
    save_dir: Optional[str] = None          # default: Graphs/<dataset>_graph_15/
...
def run_experiment(
    ...
    """Run Experiment 15 and (by default) save the Figure-16-style plot.
...
def _plot(results_df: pd.DataFrame, dataset_name: str, config: Experiment15Config) -> None:
    ...
    if config.save:
        save_dir = config.save_dir or os.path.join(
            PROJECT_ROOT, "Graphs", f"{dataset_name}_graph_15"
        )
        os.makedirs(save_dir, exist_ok=True)
        png_path = os.path.join(save_dir, f"{dataset_name}_figure16.png")
        csv_path = os.path.join(save_dir, f"{dataset_name}_figure16_results.csv")
        fig.savefig(png_path, dpi=150, bbox_inches="tight")
        results_df.to_csv(csv_path, index=False)
        print(f"  [Fig 16] saved -> {png_path}")
        print(f"  [Fig 16] saved -> {csv_path}")
